...
Define the following environment parameters:
  ...
  * `RESOURCETYPES` - List of resource types to export. Default: `ingress deployment configmap svc rc ds customresourcedefinition networkpolicy statefulset storageclass cronjob limitranges`. Notice that `Secret` objects are intentionally not exported by default (see [git-crypt section](#git-crypt) for details).
